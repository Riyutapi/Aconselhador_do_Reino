...
from comum import Obj, exibir_mensagem, nick_volume, definir_volume
...
def painel(tela: pygame.Surface):
    ...
    # Riscos marrons para parecer tábuas de madeira
    for i in range(5):
        y = (175 + (i * 25))
        altura = 250 - (i * 50)
...
def criar_arquivo(texto: str):
    ...
    """
    pasta = "save"
    if not os.path.exists(pasta):  # Se não existir
        os.makedirs(pasta)
        ...
        if abrir_painel:  # Verificar se precisa abrir o painel
            ...
            if botao_options_index == 1:  # Verificar se o painel foi aberto pelo botão das options
                ...
                if abrir_continue:  # Verificar se foi clickado o botão Continue
                    if nick is not None:  # Verificar se o arquivo save existe
                        if len(nick) <= 8:  # Se for do tamanho certo
                            escurecer(tela, volume)
                            return
